...
//    protected ParameterService paramServ = new ParameterServiceImpl();
    private static final Logger LOG = Logger.getLogger(BeforeSSIContactAuthenticator.class);
    ...
    public void authenticateImpl(AuthenticationFlowContext context) {
        try {
            ...
            String resourceUrl = uportHelperMsHost + "/connectionRequest?" + "CONTACT-DETAILS=true&" + "ssiSessionId=" + ssiSessionId + "&callback=" + callback;
            ResponseEntity<String> response
                    = restTemplate.getForEntity(resourceUrl.trim(), String.class);

            resourceUrl = uportHelperMsHost + "/connectionRequestMobile?" + "CONTACT-DETAILS=true&" + "ssiSessionId=" + ssiSessionId
