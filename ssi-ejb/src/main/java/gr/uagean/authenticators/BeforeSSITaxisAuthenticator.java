 ...
 * @author nikos
 */
public class BeforeSSITaxisAuthenticator extends AbstractSSIAuthenticator {

//    protected ParameterService paramServ = new ParameterServiceImpl();
    private static final Logger LOG = LoggerFactory.getLogger(BeforeSSITaxisAuthenticator.class);
    ...
    public void authenticateImpl(AuthenticationFlowContext context) {
        try {
            ...
            String realm = StringUtils.isEmpty(this.propServ.getProp("SSI_TAXIS_R")) ? "SSI-Taxis" : this.propServ.getProp("SSI_TAXIS_R");
            ...
            String resourceUrl = uportHelperMsHost + "/connectionRequest?" + "taxis=true&" + "ssiSessionId=" + ssiSessionId + "&callback=" + callback;
            ResponseEntity<String> response
                    = restTemplate.getForEntity(resourceUrl.trim(), String.class);

            resourceUrl = uportHelperMsHost + "/connectionRequestMobile?" + "taxis=true&" + "ssiSessionId=" + ssiSessionId
