 ...
 * @author nikos
 */
public class BeforeSSIEbillAuthenticator extends AbstractSSIAuthenticator {

//    protected ParameterService paramServ = new ParameterServiceImpl();
    private static final Logger LOG = LoggerFactory.getLogger(BeforeSSIEbillAuthenticator.class);
    ...
    public void authenticateImpl(AuthenticationFlowContext context) {
        try {
            ...
            String realm = StringUtils.isEmpty(this.propServ.getProp("SSI_EBILL_R")) ? "SSI-EBILL" : this.propServ.getProp("SSI_EBILL_R");
            ...
            String resourceUrl = uportHelperMsHost + "/connectionRequest?" + "ebill=true&" + "ssiSessionId=" + ssiSessionId + "&callback=" + callback;
            ResponseEntity<String> response
                    = restTemplate.getForEntity(resourceUrl.trim(), String.class);

            resourceUrl = uportHelperMsHost + "/connectionRequestMobile?" + "ebill=true&" + "ssiSessionId=" + ssiSessionId
